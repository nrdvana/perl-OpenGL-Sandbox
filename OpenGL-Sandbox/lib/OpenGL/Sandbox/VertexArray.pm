...
buffer can contain any record-size of data needed to pair with the shader.  This configuration
...
C<construct> step creates a cached VertexArray, and C<apply> binds it.
...
sub _bind_buffer_unless_current {
	my ($buffer, $current)= @_;
	if (ref $buffer) {
		$buffer->bind unless $buffer->id == $current;
		return $buffer->id;
	}
	else {
